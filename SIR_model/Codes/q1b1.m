...
for j = 1:length(vac_rate)  
    ...
    for i=2:length(t)
        S(i) = S(i-1) - beta*I(i-1)*S(i-1)*dt - vac_flag(i-1)*vac_rate(j)*S(i-1);
        I(i) = I(i-1) + (beta*I(i-1)*S(i-1) - alpha*I(i-1))*dt;
        if(I(i)<0.5 && ep_end==0)
            ep_end = t(i);
        end
        V(i) = V(i-1) + vac_flag(i-1)*vac_rate(j)*S(i-1);
...
ylabel('Population');
legend(['vac rate=' num2str(vac_rate(1))], num2str(vac_rate(2)), num2str(vac_rate(3)));
